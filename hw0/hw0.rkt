...
(define (kg2lb mass)
  (define m (* mass 2.2))
  (define lb (inexact->exact (floor m)))
  (define oz (inexact->exact (floor (* 16 (- m (floor m))))))
  (if (< 0 mass)
      (if (number? mass)
          (if (= lb 1)
              (if (= oz 1)
                  (list lb 'pound oz 'ounce)
                  (list lb 'pound oz 'ounces)
                  )
              (if (= oz 1)
                  (list lb 'pounds oz 'ounce)
                  (list lb 'pounds oz 'ounces)
                  )
              )
          ("number")
          )
