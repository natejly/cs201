...
(define (i-match? state symbol inst)
  (and (eq? state (ins-c-state inst))
       (eq? symbol (ins-c-symbol inst))))
...
(define (normalize config)
  (conf (conf-state config) (remove (conf-ltape config)) (conf-symbol config) (reverse (remove (reverse (conf-rtape config)))))
)
(define (remove tape)
  (cond 
  [(null? tape) tape]
  [(eq? (car tape) 'b) (remove (cdr tape))]
  [else tape]
  ))
...
(define (shift-head-left config)
  (cond
  [(empty? (conf-ltape config)) (normalize (conf (conf-state config) empty 'b (append (list (conf-symbol config)) (conf-rtape config) )))]
  [else
  (normalize (conf (conf-state config) (drop-right (conf-ltape config) 1) (last (conf-ltape config)) (append (list(conf-symbol config)) (conf-rtape config))))
  ]
...
(cond
[(empty? (conf-rtape config)) (normalize (conf (conf-state config) (append (conf-ltape config) (list (conf-symbol config))) 'b empty))]
[else
...
(define (next-config mach config)
  (let ((inst (i-lookup (conf-state config) (conf-symbol config) mach)))
    (if inst
          (cond
            [(eq? (ins-dir inst) 'L) (shift-head-left (change-state (ins-n-state inst) (write-symbol (ins-n-symbol inst) config)))]
            [(eq? (ins-dir inst) 'R) (shift-head-right (change-state (ins-n-state inst) (write-symbol (ins-n-symbol inst) config)))]
            )
        config)))
...
(define tm-convert
(list
(ins 'q1 0 'q1 0 'R) ; goes right
(ins 'q1 1 'q1 1 'R)
(ins 'q1 'b 'q2 'b 'L)

(ins 'q2 0 'q2 0 'L)
(ins 'q2 'x 'q2 'x 'L)
(ins 'q2 1 'q3 0 'R) ;one on right
(ins 'q2 'b 'q4 'b 'R)
(ins 'q3 0 'q3 1 'R)
...
; (test 'tm-sort (simulate sort (conf 'q1 '() 0 '()) 20) '(() 0 ()))
; (test 'tm-sort (simulate sort (conf 'q1 '() 1 '()) 20) '(() 1 ()))
; (test 'tm-sort (simulate sort (conf 'q1 '() 0 '(0)) 200) '(() 0 (0)))
; (test 'tm-sort (simulate sort (conf 'q1 '() 1 '(1 0)) 200) '(() 0 (1 1)))
; (test 'tm-sort (simulate sort (conf 'q1 '() 1 '(0 1 1 0 1 1)) 200) '(() 0 (0 1 1 1 1 1)))
...
(test 'tm-convert (simulate-lite tm-convert (conf 'q1 '() 1 '()) 200) '(() x ()))
(test 'tm-convert (simulate-lite tm-convert (conf 'q1 '() 1 '(1)) 200) '(() x (x x)))
(test 'tm-convert (simulate-lite tm-convert (conf 'q1 '() 1 '(1 0)) 200) '(() x (x x x x x)))
(test 'tm-convert (simulate-lite tm-convert (conf 'q1 '() 1 '(1 1 1)) 400) '(() x (x x x x x x x x x x x x x x)))
